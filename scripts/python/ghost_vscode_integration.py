...
class GhostVSCodeIntegration:
    ...
    # API Integration (Evolve server)
    def get_evolution_status_api(self) -> Dict[str, Any]:
        """Fetch evolution status from the GhostLink API server"""
        try:
            res = requests.get(f"http://localhost:8000/status", timeout=5)
            if res.status_code == 200:
                return res.json()
            return {"error": "API error", "status_code": res.status_code}
        except Exception as e:
            return {"error": str(e)}

    def trigger_evolution_api(self) -> Dict[str, Any]:
        """Trigger a single evolution generation via the API"""
        try:
            res = requests.post("http://localhost:8000/evolve", timeout=30)
    ...
    def start_background_loop_api(self, max_cycles: int = 0) -> Dict[str, Any]:
        """Start the background evolution loop via the API"""
        try:
            res = requests.post(
                f"http://localhost:8000/start-loop?max_cycles={max_cycles}", timeout=5
            )
            if res.status_code == 200:
                return res.json()
            return {"error": "API error", "status_code": res.status_code}
    ...
    def stop_background_loop_api(self) -> Dict[str, Any]:
        """Stop the background evolution loop via the API"""
        try:
            res = requests.post("http://localhost:8000/stop-loop", timeout=5)
            if res.status_code == 200:
                return res.json()
            return {"error": "API error", "status_code": res.status_code}
        except Exception as e:
    ...
    def execute_vscode_command(self, command: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(f"{self.vscode_api_url}/command", json=payload, timeout=30)

            if response.status_code == 200:
                return response.json()
            else:
                return {
                    "error": f"VS Code API error: {response.status_code}",
    ...
    def open_file_in_vscode(
        self, file_path: str, line: int = None, column: int = None
    ) -> Dict[str, Any]:
    ...
    def edit_file_in_vscode(self, file_path: str, edits: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        params = {"file": os.path.abspath(file_path), "edits": edits}
    ...
    # In-process Evolution access (preferred)
    _ei_instance: Optional[object] = None
    ...
    def get_fast_ei(self):
        """Get or create a singleton EvolutionaryIntelligence in-process instance."""
        if self._ei_instance is not None:
            return self._ei_instance
        self._ensure_sys_path()
        try:
            from src.evolutionary_intelligence import EvolutionaryIntelligence  # type: ignore
        except Exception:
            from evolutionary_intelligence import EvolutionaryIntelligence  # type: ignore
        self._ei_instance = EvolutionaryIntelligence(workspace_path=self.project_root)
        return self._ei_instance

    def get_evolution_status_local(self) -> Dict[str, Any]:
        """Return evolution status via direct in-process access."""
        try:
            ei = self.get_fast_ei()
    ...
    def trigger_evolution_local(self) -> Dict[str, Any]:
        """Trigger a single evolution generation in-process."""
        try:
            import asyncio
            ei = self.get_fast_ei()
            ok = asyncio.run(ei.evolve_generation())
            return {"success": ok, "status": ei.get_evolution_status()}
        except Exception as e:
            return {"error": str(e)}

    # API Integration (Evolve server) - optional fallback
    def get_evolution_status_api(self) -> Dict[str, Any]:
        """Fetch evolution status from the GhostLink API server"""
        try:
            res = requests.get(f"http://localhost:8000/status", timeout=5)
    ...
    def execute_ghost_command(self, command: str, *args) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd_args, cwd=self.project_root, capture_output=True, text=True, timeout=60
            )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        """Get system status preferring lowest-latency local paths"""
        ts = time.time()

        # 1) Fast local - direct in-process status
        fast_status = None
        try:
            fast_status = self.get_evolution_status_local()
            if fast_status and fast_status.get("error"):
                fast_status = None
        except Exception:
            fast_status = None

        # 2) API status (HTTP) if available (prefer /status_fast)
        api_status = None
        try:
            # Try fast endpoint first
            try:
                res = requests.get("http://localhost:8000/status_fast", timeout=3)
                if res.status_code == 200:
                    api_status = res.json().get("status")
            except Exception:
                api_status = None
            if api_status is None:
                api_status = self.get_evolution_status_api()
            if api_status and api_status.get("error"):
                api_status = None
        except Exception:
            api_status = None

        # 3) Fallback health via orchestrator subprocess
        health_result = self.execute_ghost_command("health")
        vscode_status = self.check_vscode_api()

        return {
            "timestamp": ts,
            "fast_status": fast_status or api_status,
            "ghostlink_system": {
                "healthy": health_result.get("success", False),
                "last_check": ts,
            },
            "vscode_integration": {"api_available": vscode_status, "api_url": self.vscode_api_url},
        }
    ...
    def show_system_health_in_vscode(self) -> Dict[str, Any]:
        ...
        with open(health_file, "w") as f:
            ...
            f.write(
                f"**Timestamp:** {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(health_data['timestamp']))}\n\n"
            )

            f.write("## GhostLink AI System\n")
            f.write(
                f"- **Status:** {'✅ Healthy' if health_data['ghostlink_system']['healthy'] else '❌ Issues'}\n"
            )
            f.write(
                f"- **Last Check:** {time.strftime('%H:%M:%S', time.localtime(health_data['ghostlink_system']['last_check']))}\n\n"
            )

            f.write("## VS Code Integration\n")
            f.write(
                f"- **API Available:** {'✅ Yes' if health_data['vscode_integration']['api_available'] else '❌ No'}\n"
            )
            ...
            if health_data.get("fast_status"):
                f.write("\n## Fast Status (Local/API)\n\n")
                f.write(json.dumps(health_data["fast_status"], indent=2))
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Commands: status, health, task, evolve, open, edit, terminal, git")
         sys.exit(1)
    ...
    try:
        if command == "status":
            ...
            if len(sys.argv) < 4:
                print(
                    "Usage: python3 ghost_vscode_integration.py edit <file_path> <old_string> <new_string>"
                )
                sys.exit(1)
            file_path = sys.argv[2]
            ...
            # Prefer in-process evolution for minimal latency
            result = integration.trigger_evolution_local()
            print(json.dumps(result, indent=2))
