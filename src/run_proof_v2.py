...
class GhostLinkSim:
    ...
    def encode_self(self):
        ...
        return np.array(
            [sigmas, scars, compost, ancestry_mean, activity, rho_mean, kappa_mean, integration],
            dtype=float,
        )
            ...
            for j in range(self.n):
                if self.state[i, j] == DELTA:
                    ...
                    e_sigma = (
                        ...
                        + self.theta_self_sigma * self_sum
                    )
                    e_scar = (
                        0.3 * pain
                        - 0.2 * coh
                        + 0.05 * self.rng.random()
                        + self.theta_self_scar * self_sum
                    )
    ...
    def origin_min_cut(self, steps=8, sink_threshold=0.7):
        ...
        for k, (s, _st) in enumerate(history):
            ...
            ancestry_mean = float(np.mean(self.ancestry))
            z = (
                2.0 * sig_series[min(k, len(sig_series) - 1)]
                + 0.5 * integration
                + 0.1 * ancestry_mean
            )
        ...
        for frac in (0.25, 0.5, 0.75):
            ...
            backup_rho = self.rho.copy()
            backup_kappa = self.kappa.copy()
            backup_ances = self.ancestry.copy()
            ...
            self.rho = backup_rho
            self.kappa = backup_kappa
            self.ancestry = backup_ances
            if C_cf < sink_threshold:
                return {
                    "cut_fraction": float(frac),
                    "t_sink": int(t_sink),
                    "C_sink": float(C_vals[t_sink]),
                    "C_counterfactual": float(C_cf),
                }
        return {
            "note": "no minimal cut found by proxy",
            "t_sink": int(t_sink),
            "C_sink": float(C_vals[t_sink]),
        }
...
def anomaly_index_terms(sim, window=32):
    ...
    return {
        ...
        "dL": dL.tolist(),
        "dH": dH.tolist(),
        "MI_self": float(mi_self),
        "PredLift": float(predlift),
        "chi": float(chi),
        "Gamma": float(integration),
    }
