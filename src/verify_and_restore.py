    ...
    for entry in manifest.get("hashes", []):
        ...
        if et == "dir":
            if not os.path.isdir(p):
                mismatches.append({"path": p, "reason": "missing dir"})
            else:
                act = hash_dir(p)
                if act != exp:
                    mismatches.append({"path": p, "reason": "hash mismatch"})
        elif not os.path.isfile(p):
            mismatches.append({"path": p, "reason": "missing file"})
        else:
            act = sha256_path(p)
            if act != exp:
                mismatches.append({"path": p, "reason": "hash mismatch"})
            ...
            if os.path.isdir(path):
                ...
                for fp in glob.glob(os.path.join(src, "*")):
                    if os.path.isfile(fp):
                        shutil.copy2(fp, os.path.join(path, os.path.basename(fp)))
                print(f"[restore] dir restored: {path}")
            elif os.path.isfile(src):
                os.makedirs(os.path.dirname(path), exist_ok=True)
                shutil.copy2(src, path)
                print(f"[restore] file restored: {path}")
            else:
                print(f"[restore] missing in snapshot: {src}")
        sys.exit(2)
    else:
        sys.exit(1)
