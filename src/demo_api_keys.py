...
from ghostlink.sovereign_deps import SovereignTestClient
...
def main():
    ...
    # Create test client
    client = SovereignTestClient(app)
    ...
    # Read-only key
    response = client.post("/api_keys", json={"user_id": "reader_user", "permissions": "read"})
    ...
    response = client.post(
        "/api_keys", json={"user_id": "writer_user", "permissions": "read,write"}
    )
    if response.status_code == 200:
    ...
    # Admin key
    response = client.post(
        "/api_keys", json={"user_id": "admin_user", "permissions": "read,write,admin"}
    )
    if response.status_code == 200:
    ...
    print(
        f"   External API without key: {'✗ Blocked' if response.status_code == 401 else '✓ Allowed'}"
    )
    ...
    print(
        f"   External API with read key: {'✓ Allowed' if response.status_code == 200 else '✗ Blocked'}"
    )
