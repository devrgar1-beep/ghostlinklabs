...
def get_api_key_required(x_api_key: str = Header(None, alias="X-API-Key")) -> str:
    """Extract API key from header with 400 status for missing key"""
    if not x_api_key:
...
def validate_api_key(api_key: str) -> Dict[str, Any]:
    ...
    db = get_db()
    key_obj = db.validate_api_key(api_key)
    if not key_obj:
        raise HTTPException(status_code=403, detail="Invalid or expired API key")
    return {"user_id": key_obj.user_id, "permissions": key_obj.permissions}
...
async def create_api_key(request: Dict[str, Any]):
    ...
    api_key = db.create_api_key(user_id, permissions, expires_at)
    return {
        "id": api_key.id,
        "key": api_key.key,
        "user_id": user_id,
...
async def validate_api_key_endpoint(api_key: str = Depends(get_api_key_required)):
    """Validate an API key"""
...
async def get_external_data(api_key: str = Depends(get_api_key)):
    ...
    if "admin" in key_info["permissions"]:
        ...
        # Read permission: exclude sensitive items
        filtered_items = [item for item in items if not item.get("sensitive", False)]
...
async def create_item(
    request: Dict[str, Any], api_key: str = Header(None, alias="X-API-Key")
):
    """Create an item (optional API key)"""
    item = {"name": request.get("name"), "value": request.get("value")}

    if api_key:
        key_info = validate_api_key(api_key)
        item["created_by"] = key_info["user_id"]
...
@app.get("/items")
async def get_items(api_key: str = Header(None, alias="X-API-Key")):
    """Get all items (optional API key for filtering)"""
    if api_key:
        key_info = validate_api_key(api_key)
        # Filter items: show items created by this user OR items with no created_by (public items)
        user_items = [
            item
            for item in items
            if item.get("created_by") in (None, key_info["user_id"])
        ]
        return user_items
    else:
        return {"items": items, "items_count": len(items)}


@app.post("/ipfs/store")
async def store_ipfs_data(request: Dict[str, Any]):
    """Store data in IPFS and return the hash"""
    data = request.get("data")
    if not data:
        raise HTTPException(status_code=400, detail="data is required")

    cid = ipfs.store(str(data))
    return {"cid": cid, "data": data}


@app.get("/ipfs/{cid}")
async def retrieve_ipfs_data(cid: str):
    """Retrieve data from IPFS by hash"""
    data = ipfs.retrieve(cid)
    if data is None:
...
@app.post("/reasoning/")
async def reasoning_endpoint(request: Dict[str, Any]):
    """Simple reasoning endpoint for backward compatibility"""
    text = request.get("text", "")
    return {"result": f"Reasoned about: {text}"}
...
class GhostLinkUnifiedSystem:
    ...
    async def cold_boot_system(self) -> bool:
        ...
        try:
            ...
            validate_system_operation(
                "network_initialization", {"phase": "fiber_network_boot"}
            )
            ...
            for name, role in agents_config:
                ...
                fiber_network.register_agent(
                    agent.name, {"role": role, "type": "autonomous_agent"}
                )
                validate_system_operation(
            ...
            validate_system_operation(
                "model_initialization", {"model_type": "ghostlink_custom"}
            )
            print("✅ Custom model ready (fallback mode active if needed)")

            # Final governance validation
            validate_system_operation(
                "system_boot_complete", {"system_status": "operational"}
            )
    ...
    async def demonstrate_autogen_conversation(self):
        ...
        assistant2 = AssistantAgent(
            "system_architect", "You are a system architecture specialist"
        )
        ...
        # Run conversation
        security_topic = (
            "Design a comprehensive security monitoring system for AI agents"
        )
    ...
    async def generate_final_report(self):
        ...
        print(
            f"Governance Compliance: {governance_status['overall_compliance_rate']:.1%}"
        )
