...
class SandboxEnvironment:
    ...
    def __init__(self, policy: Optional[SandboxPolicy] = None):
        # Apply profile-driven overrides when available (low-latency or open modes)
        from ..utils import perf_config

        overrides = perf_config.sandbox_overrides()
    ...
    def _install_security_hooks(self):
        ...
        # If running in 'open' profile, expand allowed syscalls and relax blocked paths
        try:
            from ..utils import perf_config

            if perf_config.is_open():
                # Add a few more syscalls typically used in integration scenarios
                self.policy.allowed_syscalls.update({
                    "socket", "connect", "sendto", "recvfrom", "getaddrinfo"
                })
            ...
            # If perf_config cannot be loaded for any reason, keep conservative defaults
            self.policy.blocked_paths = [
                "/etc/passwd", "/etc/shadow", "/root", "/home",
                "/sys", "/proc/sys", "/boot", "/usr/bin", "/usr/sbin"
            ]
