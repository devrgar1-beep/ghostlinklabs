...
def cmd_scan():
    ...
    pd.DataFrame({"priority": range(1, N + 1), "missing_marker_line": lines[:N]}).to_csv(
        out, index=False
    )
...
def forge_range(a, b):
    ...
    if not os.path.exists(csvp):
        raise SystemExit("[autoforge] run 'scan' first.")
    df = pd.read_csv(csvp)
    lines = df["missing_marker_line"].astype(str).tolist()
    a = max(1, int(a))
    b = min(int(b), len(lines))
    outdir = os.path.join(ROOT, f"auto_stubs_{a}_{b}")
...
def expand_manifest():
    ...
    try:
        ...
        manifest = {
            "name": "ghostlinklabs_pristine_bundle",
            "hashes": [],
            "whitelist": {"scripts": []},
        }
    artifacts = (
        [
            os.path.join(ROOT, "macros.vault"),
            os.path.join(ROOT, "persona.vault"),
            os.path.join(ROOT, "vault_manager.py"),
            os.path.join(ROOT, "ghostknife.py"),
            os.path.join(ROOT, "integrity_monitor.py"),
            os.path.join(ROOT, "verify_and_restore.py"),
            os.path.join(ROOT, "ghostlink_fill_queue.csv"),
            os.path.join(ROOT, "ghostlink_fill_queue_full.csv"),
        ]
        + glob.glob(os.path.join(ROOT, "auto_stubs_*"))
        + glob.glob(os.path.join(ROOT, "auto_stubs_*.zip"))
    )
    new_hashes = []
    for a in artifacts:
        if not os.path.exists(a):
            continue
        if os.path.isdir(a):
            new_hashes.append({"path": a, "sha256": hash_dir(a), "type": "dir"})
        else:
            new_hashes.append({"path": a, "sha256": sha256_path(a), "type": "file"})
    manifest["hashes"] = [
        h for h in manifest.get("hashes", []) if h["path"] not in {x["path"] for x in new_hashes}
    ] + new_hashes
    wl = set(manifest.get("whitelist", {}).get("scripts", []))
    wl.update(
        ["vault_manager.py", "ghostknife.py", "integrity_monitor.py", "verify_and_restore.py"]
    )
...
def main():
    ...
    ap.add_argument("command", choices=["scan", "autoforge", "manifest", "checkpoint"])
    ...
    if ns.command == "scan":
        ...
        if len(ns.args) != 2:
            raise SystemExit("usage: ghostknife.py autoforge <start> <end>")
