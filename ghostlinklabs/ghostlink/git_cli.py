...
def status():
    ...
    click.echo(
        f"Uncommitted Changes: {click.style('Yes' if status['has_changes'] else 'No', fg='yellow' if status['has_changes'] else 'green')}"
    )

    click.echo("\nAuto-Git Configuration:")
    click.echo(
        f"  Auto-pull: {click.style('Enabled' if auto_git.auto_pull_enabled else 'Disabled', fg='green' if auto_git.auto_pull_enabled else 'red')}"
    )
    click.echo(
        f"  Auto-merge: {click.style('Enabled' if auto_git.auto_merge_enabled else 'Disabled', fg='green' if auto_git.auto_merge_enabled else 'red')}"
    )
    click.echo(
        f"  Auto-resolve conflicts: {click.style('Enabled' if auto_git.auto_resolve_conflicts else 'Disabled', fg='green' if auto_git.auto_resolve_conflicts else 'red')}"
    )
    ...
    if auto_git.operation_history:
        ...
        for op in auto_git.operation_history[-5:]:
            ...
            click.echo(
                f"  {op.timestamp.strftime('%H:%M:%S')} - {click.style(op.status.value, fg=status_color)}: {op.message}"
            )
...
@click.option(
    "--auto-resolve/--no-auto-resolve", default=None, help="Enable/disable auto-resolve conflicts"
)
@click.option(
    "--strategy",
    type=click.Choice(["ours", "theirs", "recursive", "auto"]),
    help="Default merge strategy",
)
