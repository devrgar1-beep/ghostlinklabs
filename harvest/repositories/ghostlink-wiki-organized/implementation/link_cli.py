...
def bios_write(setting_name: str, value: str, simulate: bool, confirm: bool, hardware: bool):
    ...
    if hardware:
        if is_virtual_machine():
            click.echo("⚠️ VM detected. Hardware binding is not supported in virtualized environments. Aborting.")
            return
        if not is_admin():
            click.echo("⚠️ Administrator privileges required to bind to hardware. Please run as elevated user.")
            return
    bridge = supergrok_bios.bridge
