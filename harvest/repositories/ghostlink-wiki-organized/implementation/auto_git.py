...
class AutoGit:
    ...
    async def auto_pull(self, branch: str | None = None) -> GitOperationResult:
        ...
        try:
            ...
            result = self._run_git_command("rev-list", f"HEAD..origin/{current_branch}", "--count")
    ...
    async def auto_merge(
        ...
        try:
            ...
            # Check for conflicts
            if (
                "conflict" in merge_result.stderr.lower()
                or "conflict" in merge_result.stdout.lower()
            ):
                conflicts = self._detect_conflicts()
                ...
                if self.auto_resolve_conflicts:
                    ...
                    if resolved:
                        result = GitOperationResult(
                            ...
                            conflicts=conflicts,
                            changes={"source": source_branch, "target": target, "resolved": True},
                            timestamp=datetime.now(),
                            auto_resolved=True,
                        )
                        self.operation_history.append(result)
    ...
    async def _auto_resolve_conflicts(self, conflict_files: list[str]) -> bool:
        ...
        for file_path in conflict_files:
            try:
                ...
                if "<<<<<<<" in content and ">>>>>>>" in content:
                    ...
                    result = self._run_git_command("checkout", "--ours", str(file_path))
                    if result.returncode == 0:
        ...
        if resolved_count == len(conflict_files):
            # Complete the merge
            commit_result = self._run_git_command("commit", "--no-edit")
