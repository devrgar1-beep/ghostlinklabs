...
class TestApiKeyManagement:
    ...
    def test_create_api_key_with_expiration(self):
        ...
        payload = {"user_id": "test_user", "permissions": "admin", "expires_at": future_date}
        response = client.post("/api_keys", json=payload)
        assert response.status_code == 200
...
class TestApiKeyAuthentication:
    ...
    def setup_method(self):
        ...
        self.read_key_response = client.post(
            "/api_keys", json={"user_id": "read_user", "permissions": "read"}
        )
        self.read_key = self.read_key_response.json()["key"]

        self.write_key_response = client.post(
            "/api_keys", json={"user_id": "write_user", "permissions": "read,write"}
        )
        self.write_key = self.write_key_response.json()["key"]

        self.admin_key_response = client.post(
