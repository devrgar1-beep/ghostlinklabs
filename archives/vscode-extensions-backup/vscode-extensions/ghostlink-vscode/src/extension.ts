...
export function activate(context: vscode.ExtensionContext) {
    ...
    // New: Query the evolution API directly for fast status
    context.subscriptions.push(
        vscode.commands.registerCommand('ghostlink.apiStatus', async () => {
            try {
                const resp = await axios.get('http://localhost:8000/status');
                await displayResults(resp.data, 'apiStatus');
            } catch (err) {
                vscode.window.showErrorMessage('Failed to fetch API status: ' + String(err));
            }
        })
    );
