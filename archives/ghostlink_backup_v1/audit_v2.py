...
class SystemAuditor:
    ...
    def _audit_security(self):
        ...
        critical_findings = len([f for f in self.findings if f.severity == "critical" and f.category == "security"])
        high_findings = len([f for f in self.findings if f.severity == "high" and f.category == "security"])
    ...
    def _check_dependency_vulnerabilities(self) -> Optional[AuditFinding]:
        ...
        if not req_file.exists():
            return AuditFinding(
                ...
                evidence=["requirements.txt missing"],
    ...
    def _check_backups(self):
        ...
        backup_dirs = ["./archive", "./backup", "./colonies"]
    ...
    def _generate_report(self) -> Dict:
        ...
        overall_score = max(0, 100 - (total_risk / max_possible_risk * 100))
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if critical_findings:
            recommendations.append(f"ADDRESS IMMEDIATELY: {len(critical_findings)} critical/high severity issues")
        
        # Security
        security_findings = [f for f in self.findings if f.category == "security"]
        if security_findings:
            recommendations.append(f"Enhance security: {len(security_findings)} security issues found")
