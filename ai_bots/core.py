...
class BotContext:
    ...
    auto_commit: bool = False  # Auto-commit after every action (default OFF for safety)


class AIBot(ABC):
    """Base class for AI bots"""

    def __init__(self, name: str, access_level: AccessLevel = AccessLevel.READ):
        self.name = name
    ...
    async def execute(self, command: str, args: List[str], context: BotContext) -> Dict[str, Any]:
        """Execute a command"""
        ...
        if cmd.requires_confirmation and not context.yolo_mode:
            return {"confirm": f"Are you sure you want to run '{cmd.name}'? (y/N)"}
    ...
    def _check_access(self, required: AccessLevel, provided: AccessLevel) -> bool:
...
class SystemBot(AIBot):
    ...
    async def initialize(self):
        ...
        self.register_command(
            "status", self.cmd_status, "Show system status", AccessLevel.READ, hotkey="ctrl+shift+s"
        )
        ...
        self.register_command(
            "logs", self.cmd_logs, "View system logs", AccessLevel.ADMIN, hotkey="ctrl+shift+l"
        )
    ...
    async def cmd_restart(self, args: List[str], ctx: BotContext) -> str:
        ...
        import subprocess
        # Anti-drift check: ask DriftGuard to analyze this action
        try:
            from ghostlink.modules.driftguard import default as driftguard_module
            findings = driftguard_module.analyze({"action": "restart", "service": service})
            if findings.get("findings"):
                # Block unless YOLO mode
                if not ctx.yolo_mode:
                    return f"Blocked by DriftGuard: {findings['findings']}"
        except Exception:
            # If DriftGuard fails, continue but log could be improved
            pass
        ...
        try:
            ...
            return f"Cannot restart {service}: " "Neither docker-compose nor systemctl available"
        except Exception as e:
    ...
    async def cmd_logs(self, args: List[str], ctx: BotContext) -> str:
        ...
        try:
            ...
            return f"No logs found for {service}. " "Tried Docker, systemd, and common log paths."
    ...
    async def cmd_exec(self, args: List[str], ctx: BotContext) -> str:
        ...
        # Anti-drift check: analyze exec attempt
        try:
            from ghostlink.modules.driftguard import default as driftguard_module
            findings = driftguard_module.analyze({"action": "exec", "command": " ".join(args)})
            if findings.get("findings"):
                if not ctx.yolo_mode:
                    return f"Blocked by DriftGuard: {findings['findings']}"
        except Exception:
    ...
    async def cmd_yolo(self, args: List[str], ctx: BotContext) -> str:
        ...
        action = "be auto-approved" if ctx.yolo_mode else "require confirmation"
    ...
    async def cmd_autocommit(self, args: List[str], ctx: BotContext) -> str:
        ...
        action = "commit after every action" if ctx.auto_commit else "not auto-commit"
...
class MetricsBot(AIBot):
    ...
    async def initialize(self):
        ...
        self.register_command("get", self.cmd_get, "Get metrics", AccessLevel.READ, hotkey="ctrl+m")

        self.register_command(
            "alert", self.cmd_alert, "Create alert rule", AccessLevel.ADMIN, aliases=["watch"]
        )
    ...
    async def cmd_get(self, args: List[str], ctx: BotContext) -> str:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get("http://127.0.0.1:9108/metrics", timeout=2.0)
...
class CommandRouter:
    ...
    async def route(self, input_str: str, context: BotContext) -> Dict[str, Any]:
    ...
    async def handle_hotkey(self, hotkey: str, context: BotContext) -> Dict[str, Any]:
        """Handle hotkey press"""
...
async def main():
    ...
    # Create root context
    context = BotContext(user_id="admin", session_id="local", access_level=AccessLevel.ROOT)
