...
class ThoughtStream:
    ...
    def think(
        ...
        """
        thought = Thought(
            type=thought_type, content=content, confidence=confidence, metadata=metadata or {}
        )
    ...
    def _print_thought(self, thought: Thought):
        ...
        print(f"{icon} [{thought.type.value.upper()}] " f"({confidence_bar}) {thought.content}")

    def get_recent(self, n: int = 10, thought_type: Optional[ThoughtType] = None) -> List[Thought]:
        """
        Get recent thoughts
    ...
    def summarize(self) -> Dict[str, Any]:
        ...
        avg_confidence = (
            sum(t.confidence for t in self.thoughts) / len(self.thoughts) if self.thoughts else 0.0
        )
    ...
    def _analyze_state(self, state: SystemState):
        ...
        if not state.is_healthy():
            ...
            if bottleneck == "Memory":
                self.thought_stream.think(
                    ThoughtType.HYPOTHESIS,
                    "High memory usage may impact performance. " "Consider releasing resources.",
                    confidence=0.8,
                )
            elif bottleneck == "CPU":
                self.thought_stream.think(
                    ThoughtType.HYPOTHESIS,
                    "High CPU usage detected. " "May need to throttle operations.",
                    confidence=0.8,
                )
...
class MetacognitiveAgent:
    ...
    def __init__(self, name: str, groq_api_key: Optional[str] = None, verbose: bool = False):
        """
    ...
    async def reflect(self, prompt: str, use_ai: bool = True) -> Dict[str, Any]:
        """
        ...
        """
        # Observe
        self.thought_stream.think(ThoughtType.OBSERVATION, f"Received prompt: {prompt[:100]}...")

        # Check system state
        state = self.system_awareness.get_current_state()
        self.thought_stream.think(
            ThoughtType.SYSTEM_STATE,
            f"System: CPU {state.cpu_percent}%, " f"Memory {state.memory_percent}%",
            metadata=state.to_dict(),
        )

        # Generate plan
        self.thought_stream.think(ThoughtType.PLAN, "Will analyze prompt and generate response")

        # Generate response
        if use_ai and self.groq_api_key:
            response = await self._ai_reflect(prompt)
        else:
            response = f"Reflection on: {prompt}\n" f"System state: {state.to_dict()}"

        # Decision
        self.thought_stream.think(
            ThoughtType.DECISION, "Generated reflection response", confidence=0.9
        )

        return {
            "response": response,
            "thoughts": [t.to_dict() for t in self.thought_stream.get_recent(10)],
    ...
    async def _ai_reflect(self, prompt: str) -> str:
        ...
        try:
            ...
            self.thought_stream.think(
                ThoughtType.DOUBT, f"AI reflection error: {e}", confidence=0.5
            )
            return f"Error: {e}"
    ...
    def get_introspection_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "system_trends": (self.system_awareness.get_trend_analysis()),
...
async def demo():
    ...
    result = await agent.reflect("What is the nature of consciousness in artificial systems?")
