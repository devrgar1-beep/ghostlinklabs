...
SCAR_WINDOWS = Counter(
    "ghostlink_scar_windows_total", "Cumulative windows containing a scar", ["roi"]
)
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.listen(1)
        print(
            f"[controller] listening on {HOST}:{PORT} (WIN={WIN}s GOOD_MAX={GOOD_MAX}C SCAR_HOT={SCAR_HOT}C SCAR_SUSTAIN={SCAR_SUSTAIN}s)"
        )
        conn, addr = s.accept()
        with conn:
