...
This repository contains the unified GhostLink codebase, consolidating multiple development branches and versions into a single, coherent structure.

### Directory Organization

- **`ghostlink/`** - Core Python package with all modules
  - `core/` - Main coordination logic and FCC lattice implementation
  - `lattice/` - Agent topology and spatial reasoning
  - `reasoning/` - CMFL cycle implementation
  - `mesh/` - Stigmergic coordination mechanisms
  - `opcode/` - Low-level instruction set
  - `forge/` - Code generation and transformation tools
  - `daemon/` - Background services and orchestration
  - `interfaces/` - API and integration layers

- **`docs/`** - Documentation, specifications, and research papers
  - Markdown files, PDFs, whitepapers, and technical specifications
  - Includes consolidated documentation from all development versions
...
- **`node/`** - Node.js components and frontend
  - React dashboard and monitoring interfaces
  - WebSocket servers for real-time coordination

- **`python/`** - Python-specific deployment and utilities
  - Additional Python tools and scripts
  - Environment configurations

- **`archives/`** - Historical and backup materials
  - Legacy implementations and research artifacts
  - Experimental code and alternative implementations

### Development Workflow

1. **Core Development**: Work in `ghostlink/` package
2. **Documentation**: Add to `docs/` directory
3. **Testing**: Use `tests/` and integration scripts
4. **Deployment**: Utilize `docker/`, `node/`, and `python/` configurations
